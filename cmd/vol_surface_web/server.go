...
func runWebServer() {
	...
	// Load data
	options, err := LoadOptionsFromCSV("eth_deribit_options.csv")
	if err != nil {
	...
	for i := range options {
		if !math.IsNaN(options[i].MidETH) && options[i].MidETH > 0 {
			iv := CalculateImpliedVolatility(options[i], spotPrice, riskFreeRate)
	...
	// Create analyzer
	analyzer := NewVolSurfaceAnalyzer(cleanedOptions, spotPrice, riskFreeRate)
