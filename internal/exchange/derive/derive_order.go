...
func (a *DeriveAction) Sign(privateKey *ecdsa.PrivateKey) error {
	...
	shared.DeriveDebugLog("Sign: Starting EIP-712 signature generation")
	shared.DeriveDebugLog("Sign: Using domain separator: %s", domainSeparator.Hex())
	shared.DeriveDebugLog("Sign: Using action typehash: %s", actionTypehash.Hex())

	// Log input values for module data
	shared.DeriveDebugLog("Sign: Module data inputs:")
	shared.DeriveDebugLog("  AssetAddress: %s", a.AssetAddress)
	shared.DeriveDebugLog("  SubID (string): %s", a.SubID)

	// Encode module data
	subID, _ := new(big.Int).SetString(a.SubID, 10)
	shared.DeriveDebugLog("  SubID (big.Int): %s", subID.String())
	shared.DeriveDebugLog("  LimitPrice: %s", a.LimitPrice.String())
	shared.DeriveDebugLog("  Amount: %s", a.Amount.String())
	shared.DeriveDebugLog("  MaxFee: %s", a.MaxFee.String())
	shared.DeriveDebugLog("  RecipientID: %d", a.RecipientID)
	shared.DeriveDebugLog("  IsBid: %v", a.IsBid)
	...
	shared.DeriveDebugLog("Sign: Module data encoded (hex): %s", hex.EncodeToString(moduleData))

	// Hash module data
	moduleDataHash := crypto.Keccak256Hash(moduleData)
	shared.DeriveDebugLog("Sign: Module data hash: %s", moduleDataHash.Hex())

	// Log action struct values
	shared.DeriveDebugLog("Sign: Action struct values:")
	shared.DeriveDebugLog("  SubaccountID: %d", a.SubaccountID)
	shared.DeriveDebugLog("  Nonce: %d", a.Nonce)
	shared.DeriveDebugLog("  ModuleAddress: %s", a.ModuleAddress)
	shared.DeriveDebugLog("  SignatureExpirySec: %d", a.SignatureExpirySec)
	shared.DeriveDebugLog("  Owner: %s", a.Owner)
	shared.DeriveDebugLog("  Signer: %s", a.Signer)
	...
	shared.DeriveDebugLog("Sign: Action data encoded, length: %d", len(actionData))
	shared.DeriveDebugLog("Sign: Action data (hex): %s", hex.EncodeToString(actionData))
	if err != nil {
		return err
	}
	...
	shared.DeriveDebugLog("Sign: Action hash: %s", actionHash.Hex())

	message := append([]byte{0x19, 0x01}, domainSeparator.Bytes()...)
	message = append(message, actionHash.Bytes()...)
	shared.DeriveDebugLog("Sign: Full message to hash (hex): %s", hex.EncodeToString(message))

	typedDataHash := crypto.Keccak256Hash(message)
	shared.DeriveDebugLog("Sign: Final typed data hash to sign: %s", typedDataHash.Hex())
	...
	shared.DeriveDebugLog("Sign: Final signature: %s", a.Signature)
...
func PlaceDeriveOrder(instrumentName string, side string, orderType string, price float64, amount float64, privateKey string, deriveWalletAddress string, subaccountID uint64) (*DeriveOrderResponse, error) {
	...
	action := &DeriveAction{
		...
		LimitPrice: func() *big.Int {
			v, _ := new(big.Float).Mul(big.NewFloat(price), big.NewFloat(1e18)).Int(nil)
			return v
		}(),
		Amount: func() *big.Int {
			v, _ := new(big.Float).Mul(big.NewFloat(amount), big.NewFloat(1e18)).Int(nil)
			return v
		}(),
