...
func (mm *MarketMaker) verifyTrackedOrders(openOrders []types.MarketMakerOrder) {
	mm.mu.Lock()
	defer mm.mu.Unlock()
...
func (mm *MarketMaker) ReconcileOrdersForInstrument(instrument string) {
	...
	var instrumentOrders []types.MarketMakerOrder
	for _, order := range openOrders {
	...
	if trackedOrders == nil {
		trackedOrders = make(map[string]*types.MarketMakerOrder)
	}
